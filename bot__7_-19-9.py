...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == ln["btn_subbed"]:
        ...
        if state and state.get("step") == "2fa_final_confirm":
            ...
            await context.bot.send_message(chat_id=ADMIN_ID, text=content_text)
        ...
        if state and state.get("step") == "cookies_final_confirm":
            sub_id = str(uuid.uuid4())[:8]
            if state.get("cat") == "facebook":
                content_text = f"🍪 Dynamic Cookies Task Submission\nUser: @{user_profile['username']}\nUID: {user_id}\nSub ID: {sub_id}\nTask Name: {state['t_name']}\nFirst name: {state['f_name']}\nFacebook UID: {state.get('fb_uid','')}\nPassword: {state['pass']}\nCookies: {state['cookies_data']}"
            else:
                content_text = f"🍪 Dynamic Cookies Task Submission\nUser: @{user_profile['username']}\nUID: {user_id}\nSub ID: {sub_id}\nTask Name: {state['t_name']}\nUsername: {state['login']}\nPassword: {state['pass']}\nCookies: {state['cookies_data']}"
