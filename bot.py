...
from telegram import Update, ReplyKeyboardMarkup, KeyboardButton as _TelegramKeyboardButton, InlineKeyboardMarkup, InlineKeyboardButton as _TelegramInlineKeyboardButton
...
def InlineKeyboardButton(text, *args, **kwargs):
    """Create an inline button and automatically attach the matching Premium emoji."""
...
def KeyboardButton(text, *args, **kwargs):
    """Create a reply-keyboard button with its matching Premium custom emoji icon."""
    if "icon_custom_emoji_id" not in kwargs:
        emoji_id = _premium_button_emoji(text)
        if emoji_id:
            kwargs["icon_custom_emoji_id"] = emoji_id
