...
async def admin_send_all_reports(bot, chat_id: int, db_data: dict):
    """All Report বাটনের জন্য pending সাবমিশনগুলো পাঠানোর লজিক (টেক্সট ও ইনলাইন — দুই জায়গা থেকেই কল হয়)।"""
    pending_subs = [s for s in db_data["submissions"].values() if s["status"] == "pending"]
    if not pending_subs:
        await bot.send_message(chat_id=chat_id, text="✅ No pending item reports.")
        return
    ...
    for s in pending_subs:
        ...
        await bot.send_message(chat_id=chat_id, text=report_txt, reply_markup=inline_ap)
...
async def callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID and data in [
        ...
        if data == "adm_add_task":
            ...
            await admin_send_all_reports(context.bot, user_id, db_data)
