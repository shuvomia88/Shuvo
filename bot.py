...
# Final exact Premium IDs for the Facebook-task UI supplied by the user.
# These override older/general emoji mappings above.
GLOBAL_BODY_EMOJIS.update({
    "🔜": "6089104521428997572",
    "😎": "5235807206770750407",
    "✅": "4956454790012863177",
...
# 🌟 কোনো কোনো ইমুজি (✅, ❌, ➡️ ইত্যাদি) বিভিন্ন মেসেজে বিভিন্ন প্রিমিয়াম আইডি দিয়ে ব্যবহার হয় —
# একটা সাধারণ (global) ম্যাপ দিয়ে এটা সম্ভব না, তাই নির্দিষ্ট মেসেজ শনাক্ত করে সেই মেসেজের জন্য আলাদা
# আইডি বসানোর ব্যবস্থা করা হলো। একই মেসেজে একই ইমুজি একাধিকবার থাকলে (যেমন "First name"/"Use information"
# দুই জায়গায় ✅), তালিকার আইডিগুলো ক্রমানুসারে বসবে (প্রথমটায় প্রথম আইডি, দ্বিতীয়টায় দ্বিতীয় আইডি)।
MESSAGE_EMOJI_OVERRIDES = [
    ("This task is currently closed", {
        "🚧": ["5336944168944047463"], "❌": ["5949785428843302949"],
        "✅": ["6206185428702206246"], "🔄": ["5030872266716480568"],
    }),
    ("এই টাস্কে বর্তমানে কাজ বন্ধ আছে", {
        "🚧": ["5336944168944047463"], "✅": ["6206185428702206246"],
    }),
    ("Payout: $", {
        "⭐️": ["5127861690641156032"], "🛡": ["6174589325695521740"],
        "💵": ["6206155797722830770"], "📝": ["5215672443036772796"],
        "🚀": ["6068700050928704109"],
    }),
    ("Please Send Your Facebook UID", {
        "➡️": ["5260450573768990626"],
    }),
    ("ফেসবুক ইউআইডি (Facebook UID) পাঠান", {
        "➡️": ["5260450573768990626"],
    }),
    ("Time's up! Task cancelled", {
        "🕐": ["5215484787325676090"], "❌": ["5210952531676504517"],
    }),
    ("Please Send Your Cookies Data", {
        "👉": ["6158862632926319619"], "📂": ["6017174676898321263"],
    }),
    ("কুকিজ (Cookies) ডাটা পাঠান", {
        "👉": ["6158862632926319619"],
    }),
    ("Your report has been received", {
        "✅": ["6206294254583551886"], "⌛": ["5386367538735104399"],
    }),
    ("আপনার রিপোর্টটি গ্রহণ করা হয়েছে", {
        "✅": ["6206294254583551886"],
    }),
    ("All Account Report", {
        "📊": ["5353032893096567467"], "📥": ["6203886371363364022"],
        "✅": ["5352694861990501856"], "⌛": ["5386367538735104399"],
        "❌": ["5420130255174145507"],
    }),
    ("সকল অ্যাকাউন্ট রিপোর্ট", {
        "📊": ["5353032893096567467"], "✅": ["5352694861990501856"],
        "❌": ["5420130255174145507"],
    }),
    ...
    ("First name:", {
        "✅": ["6206185428702206246", "6206294254583551886"],  # প্রথম ✅ (First name) আর দ্বিতীয় ✅ (Use information) আলাদা আইডি
        "📱": ["5848177124074066676"], "⬆️": ["5470143839371021696"],
    }),
    ("Welcome to", {
        "🔜": ["6089104521428997572"], "😎": ["5235807206770750407"],
        "✅": ["4956454790012863177"], "📤": ["5353001161878182134"],
        "💰": ["6190336264940559752"], "⚡️": ["6082511510406436819"],
        "😀": ["5978822208082939495"], "🔥": ["6068682402908083940"],
    }),
...
def premiumize_entities(text, existing_entities=None):
    ...
    # 🌟 নির্দিষ্ট মেসেজের জন্য বিশেষ আইডি থাকলে সেটা এখানে প্রস্তুত করা হচ্ছে (positional/sequential)
    active_override = None
    for marker, override in MESSAGE_EMOJI_OVERRIDES:
        if marker in text:
    ...
    for emoji, emoji_id in sorted(emoji_map.items(), key=lambda item: len(item[0]), reverse=True):
        ...
        while True:
            ...
            if not any(pos < e and end > s for s, e in occupied):
                final_id = emoji_id
                if active_override and emoji in active_override:
                    idx = override_counters.get(emoji, 0)
                    id_list = active_override[emoji]
                    final_id = id_list[min(idx, len(id_list) - 1)]
                    override_counters[emoji] = idx + 1
...
def _premiumize_button_text(text):
    """Remove normal emoji from button labels; the button icon is supplied via icon_custom_emoji_id."""
    if not isinstance(text, str):
        return text
    result = text
    all_emojis = sorted(set(PREMIUM_EMOJI_IDS) | set(GLOBAL_BODY_EMOJIS), key=len, reverse=True)
    ...
    if "entities" not in kwargs:
        if len(args) >= 2:
            ...
            if isinstance(text, str):
                kwargs["entities"] = premiumize_entities(text)
        elif isinstance(kwargs.get("text"), str):
            kwargs["entities"] = premiumize_entities(kwargs["text"])
...
LANGUAGES = {
    "en": {
        ...
        "send_fb_uid": "➡️ Please Send Your Facebook UID",
    ...
    "bn": {
        ...
        "send_fb_uid": "➡️ অনুগ্রহ করে আপনার ফেসবুক ইউআইডি (Facebook UID) পাঠান",
...
def _restore_button_text(text: str) -> str:
    ...
    candidates.extend([
        ...
        "🔗 𝖱𝖾𝖿𝖾𝗋 & 𝖤𝖺𝗋𝗇", "➕ Add Task", "❌ Delete Task",
        "👁️ Task Hide/Show", "👤 User Broadcast", "➕ Add Money",
        "📥 Username Save", "🗂️ All Report", "🗑️ User Delete",
        "🔐 Password Change", "📷 Instagram File", "📘 Facebook File",
        "🎥 Work Video", "🔙 Return to main menu", "Withdraw",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- MENU NAVIGATION ---
    if text in ["YOUR REPORT", "📊 YOUR REPORT", "📊 আপনার রিপোর্ট"]:
        await update.message.reply_text(
    ...
    if text in ["SUPPORT", "ℹ️ SUPPORT", "ℹ️ সাপোর্ট (SUPPORT)"]:
    ...
    if text in ["LANGUAGE", "🌐 LANGUAGE", "🌐 ভাষা (LANGUAGE)"]:
    ...
    if text in ["TASK", "📋 TASKS", "📋 কাজ (TASKS)"]:
        btn_ig_cat = KeyboardButton("INSTAGRAM TASK")
        btn_fb_cat = KeyboardButton("FACEBOOK TASK")
    ...
    if target_task:
        ...
        rules_entities = premiumize_entities(rules_msg)
    ...
    if text in ["WITHDRAW", "📤 WITHDRAW", "📤 টাকা তুলুন"]:
    ...
    if user_id == ADMIN_ID and text == "TASK HIDE/SHOW":
        v = db_data["visibility"]
        btn_h_ig = InlineKeyboardButton(f"📸 Instagram [{'ON' if v.get('instagram_task',True) else 'OFF'}]", callback_data="h_ig_m")
        btn_h_fb = InlineKeyboardButton(f"📘 Facebook [{'ON' if v.get('facebook_task',True) else 'OFF'}]", callback_data="h_fb_m")
        _style(btn_h_ig, 'primary')
        _style(btn_h_fb, 'success')
    ...
    if user_id == ADMIN_ID and text == "USER DELETE":
...
async def callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("h_"):
        ...
        btn_m_ig = InlineKeyboardButton(f"📸 Instagram [{'ON' if v.get('instagram_task',True) else 'OFF'}]", callback_data="h_ig_m")
        btn_m_fb = InlineKeyboardButton(f"📘 Facebook [{'ON' if v.get('facebook_task',True) else 'OFF'}]", callback_data="h_fb_m")
        _style(btn_m_ig, 'primary')
        _style(btn_m_fb, 'success')
