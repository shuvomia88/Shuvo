...
def _premiumize_button_text(text):
    """Remove normal emoji from button labels and uppercase the rest, so button text
    stays short/consistent for matching in handle_message. (No icon_custom_emoji_id is
    used here anymore — that was never a real Telegram button field and crashed
    keyboard creation; removed for good.)"""
    if not isinstance(text, str):
        return text
    result = text
...
LANGUAGES = {
    ...
    "bn": {
        ...
        "withdraw_min_err": "❌ ব্যর্থ: সর্বনিম্ন $0.20 ব্যালেন্স থাকা আবশ্যক",
...
async def send_admin_dashboard(bot, chat_id: int, lang: str):
    ...
    btn_all_r = InlineKeyboardButton("🗂️ All Report", callback_data="adm_all_report")
    ...
    btn_all_file = InlineKeyboardButton("🗃️ Bot All Task File", callback_data="adm_all_task_file")
...
async def admin_send_file_dump(bot, chat_id: int, category: str):
    """Instagram/Facebook File বাটনের জন্য Excel এক্সপোর্ট লজিক। ফাইল পাঠানোর সাথে সাথে
    ডাটা মুছে ফেলা হয় না — ফাইলের নিচে একটা Clear বাটন থাকে, সেটা চাপলেই শুধু ডাটা মুছে
    নতুন করে সেভ হওয়া শুরু হয়।"""
    ...
    headers = ["Task Name", "Type", "Username", "Password", "2FA"]
    ws.append(headers)
    ...
    for g_idx, (task_name, entries) in enumerate(grouped.items()):
        ...
        for e in entries:
            entry_type = "2FA" if e.get("twofa") else "Cookies"
            ws.cell(row=row_num, column=1, value=task_name).fill = fill
            ws.cell(row=row_num, column=2, value=entry_type).fill = fill
            ws.cell(row=row_num, column=3, value=e.get("username", "")).fill = fill
            ws.cell(row=row_num, column=4, value=e.get("password", "")).fill = fill
            ws.cell(row=row_num, column=5, value=e.get("twofa", "")).fill = fill
    ...
    btn_clear = InlineKeyboardButton("🗑️ Clear", callback_data=f"adm_clear_dump:{category}")
...
async def admin_process_report_uids(bot, admin_chat_id: int, uid_list: list, approve: bool):
    """
    .txt ফাইলে দেওয়া UID লিস্ট থেকে প্রতিটা ইউজারের সব pending সাবমিশন
    bulk approve/reject করে, ব্যালেন্স/কাউন্ট আপডেট করে, এবং প্রতিটা
    ইউজারকে নোটিফাই করে।
    """
    processed = []
    not_found = []
    with _lock:
        ...
        for raw_uid in uid_list:
            ...
            if not uid:
                continue
            user_subs = [s for s in d["submissions"].values() if str(s["user_id"]) == uid and s["status"] == "pending"]
            if not user_subs:
            ...
            if uid not in d.get("users", {}):
                ...
                d.setdefault("users", {})[uid] = {
                    "user_id": uid_int, "username": "", "balance": 0.0, "language": "bn",
                    "success_count": 0, "review_count": 0, "rejected_count": 0,
                }
            for s_rec in user_subs:
                if approve:
    ...
    for uid in processed:
        ...
        if approve:
            u_msg = "✅ রিপোর্ট গৃহীত হয়েছে! নতুন ব্যালেন্স যোগ হয়েছে।" if u_lang == "bn" else "✅ Your report has been approved! Balance credited."
        else:
    ...
    summary = (
        f"✅ প্রসেস সম্পন্ন!\n\n"
        f"{status_word} হয়েছে: {len(processed)} জন\n"
        f"খুঁজে পাওয়া যায়নি / pending নেই: {len(not_found)} জন"
    )
    if not_found:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID:
        btn_admin_inline = InlineKeyboardButton("🛠️ Admin Panel", callback_data="open_admin_panel")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in [ln["btn_tasks"], "TASK", "📋 TASKS", "📋 কাজ (TASKS)"]:
        btn_ig_cat = KeyboardButton(ln["cat_instagram"])
        btn_fb_cat = KeyboardButton(ln["cat_facebook"])
...
async def callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID and data in [
        ...
        "adm_work_video", "adm_all_task_file", "adm_pending_uids", "adm_back",
    ]:
        if data == "adm_add_task":
            ...
            ig_count = len(db_data.get("ig_dump", []))
            fb_count = len(db_data.get("fb_dump", []))
            btn_fb_f = InlineKeyboardButton(f"📘 Facebook File Total: {fb_count:02d}", callback_data="adm_fb_file")
            btn_ig_f = InlineKeyboardButton(f"📷 Instagram File Total: {ig_count:02d}", callback_data="adm_ig_file")
            _style(btn_fb_f, 'primary')
            _style(btn_ig_f, 'primary')
            kb = InlineKeyboardMarkup([[btn_fb_f], [btn_ig_f]])
            await query.message.reply_text("🗃️ কোন ক্যাটাগরির সাবমিশন ফাইল দেখতে চান?", reply_markup=kb)
            ...
            if not pending_subs:
                ...
                for s in pending_subs:
                    u_name = s.get("username") or "N/A"
                    t_type = "2FA" if s.get("task_type") == "2fa" else "Cookies"
                    cat = "Facebook" if s.get("category") == "facebook" else "Instagram"
                    lines.append(f"🆔 UID: {s['user_id']}  |  @{u_name}  |  {cat} {t_type}")
                text_out = "\n".join(lines)
    ...
    if user_id == ADMIN_ID and data in ["adm_ig_file", "adm_fb_file"]:
        await admin_send_file_dump(context.bot, user_id, "instagram" if data == "adm_ig_file" else "facebook")
        return
    ...
    if user_id == ADMIN_ID and data == "adm_report_success":
        ...
        await query.message.reply_text("📤 Upload your success ID txt (প্রতি লাইনে একটা করে ইউজার UID)")
        return

    if user_id == ADMIN_ID and data == "adm_report_rejected":
        USER_STATE[user_id] = {"step": "waiting_report_txt", "report_mode": "reject"}
        await query.message.reply_text("📤 Upload your rejected ID txt (প্রতি লাইনে একটা করে ইউজার UID)")
        return
